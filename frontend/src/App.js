...
function OldApp() {
  ...
  const [activeTab, setActiveTab] = useState("security"); // "security" or "threat"
  ...
  // Extract relevant security findings
  ...
  // Ensure editor mounts with controlled scroll behavior
  ...
  const handleEditorDidMount = (editor) => {
    ...
    // Disable default scrolling behavior and force top
    const scrollInfo = editor.getScrollInfo();
    editor.scrollTo(0, 0);
    editor.setCursor({ line: 0, ch: 0 });
    
    // Force refresh to apply layout
    setTimeout(() => {
      editor.refresh();
      editor.scrollTo(0, 0);
    }, 10);
    
    // After any change, prevent auto-scroll to cursor
  ...
  // Fallback: if first code load is large (likely pasted), ensure top
  useEffect(() => {
    if (cmRef.current && code.length > 0) {
      const info = cmRef.current.getScrollInfo();
      // If scrolled near bottom after paste, pull back to top
      if (info.top > 100) {
          ...
          <div style={{ marginBottom: "15px", display: "flex", gap: "15px", alignItems: "center" }}>
            ...
            <div style={{ marginLeft: "auto", display: "flex", gap: "10px" }}>
              <button
                ...
                onMouseOver={(e) => !loading && code.trim() && (e.target.style.backgroundColor = "#c0392b")}
                onMouseOut={(e) => !loading && code.trim() && (e.target.style.backgroundColor = "#e74c3c")}
              >
              ...
              <button
                ...
                onMouseOver={(e) => !loadingThreat && code.trim() && (e.target.style.backgroundColor = "#8e44ad")}
                onMouseOut={(e) => !loadingThreat && code.trim() && (e.target.style.backgroundColor = "#9b59b6")}
              >
        ...
        {threatModel && activeTab === "threat" && (
          <div style={{
            ...
            <div style={{
              ...
              <strong>💡 What is STRIDE?</strong><br/>
              STRIDE is a threat modeling framework that categorizes security threats into 6 types:
              <strong> Spoofing</strong>, <strong>Tampering</strong>, <strong>Repudiation</strong>, 
              <strong> Information Disclosure</strong>, <strong>Denial of Service</strong>, and 
